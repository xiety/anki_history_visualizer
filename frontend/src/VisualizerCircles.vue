<template>
    <div class="parameters">
        <Parameter name="Ball size" v-model="ball_size" :step="1" :min="1" :max="200" />
        <Parameter name="Day height" v-model="day_height" :step="0.01" :min="0.1" :max="10" />
...
watch([() => props.slider_value, () => props.records, ball_size, day_height, max_stability], refresh_canvas);
...
function generate() {
    const step_width = (props.width - box_width * 2) / props.records.length;
    let x = box_width;
    circles = []
...
function draw() {
    ...
    const lines_num = props.height / pixels_between_lines;

    for (let i = 1; i < lines_num; ++i) {
        const y = props.height - (i * pixels_between_lines);
        ...
        ctx.value.fillText(`${d}`, props.width - 20, y - 4, 20);
    }
